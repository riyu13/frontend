...
const HirakataList = () => {
  ...
  const filteredHirakata = hirakata.filter(
    (hirakata) =>
      hirakata.hiragana.toLowerCase().includes(searchTerm.toLowerCase()) ||
      hirakata.katakana.toLowerCase().includes(searchTerm.toLowerCase()) ||
      hirakata.romaji.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div>
      ...
      <Link to="/hirakata/add" className="button is-primary mb-2">
        Tambah
      </Link></div>
      {/* Input pencarian */}
      ...
      <input
        className='input'
        type="text"
        placeholder="Cari hiragana, katakana, atau romaji..."
        value={searchTerm}
      ...
      <table className="table is-striped is-bordered is-hoverable is-fullwidth" style={{ backgroundColor: '#FFFFE0' }}>
        ...
        <tbody>
          {filteredHirakata.map((hirakata, index) => (
            <tr key={hirakata.uuid}>
              ...
              <td>
                <Link to={`/hirakata/edit/${hirakata.uuid}`} className="button is-small is-info">
                  <FaEdit />
                </Link>
                <button onClick={() => deleteHirakata(hirakata.uuid)} to={`/hirakata/edit/${hirakata.uuid}`} className="button is-small is-danger">
                  <FaTrash />
                </button>
              </td>
            </tr>
